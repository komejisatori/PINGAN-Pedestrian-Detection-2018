...
def compute_ap(rec, prec):
    ...
    plt.step(mrec, mpre, color='b', alpha=0.2,
             where='post')
    # plt.fill_between(mrec, mpre, step='post', alpha=0.2, color='b')
...
def pedestrian_eval(input, gt_file, ignore_file, ove):
    ...
    for ovthresh in ove:
        ...
        iou = iou_sum / float(npos)
        ap = compute_ap(rec, prec)
        aap.append(ap)

        Threshs.append(ovthresh)
        Recalls.append(tp_count / npos)
        AIOUs.append(iou)
        APs.append(ap)

        print("Threshold: {:.2f}".format(ovthresh))
        print("TP Count: " + str(tp_count))
        print("NPOS Count: " + str(npos))
        # print("FN Count: " + str(npos - tp_count))
        print("FP Count: " + str(fp_count))
        print("FP Count Per Images: " + str(fp_count / 5000))
        print("Recall: " + str(tp_count / npos))
...
def wider_ped_eval(input, gt, ignore_file, ove):
    aap = pedestrian_eval(input, gt, ignore_file, ove)
    mAP = np.average(aap)
    return mAP
...
if __name__ == '__main__':
    ...
    mAP = wider_ped_eval(submit_file, gt_file, ignore_file, ove)
    # Plot
    plt.title("Score " + str(mAP) + " Result Analysis [" + str(ove) + " : 0.5 : " + str(MAX_OVE) + "]")
    # plt.plot(Threshs, Recalls, 'ro-', label="Recalls")
    # plt.plot(Threshs, AIOUs, 'g^-', label="Average IOU")
    plt.plot(Threshs, APs, 'bs-', label="AP")

    plt.legend()
    plt.show()

    out = {'Average AP @' + str(ove) + ' to ' + str(MAX_OVE): mAP}
